package  fedres.dashboard.controller
{
	 ...
	 * 
	 */
	public class LoadDashboardCommand extends  Command
	{
		[Inject] public var model: EconomyDashboardModel;
		[Inject] public var fred:  FredAPIService;		
		[Inject] public var event:    LoadDashboardTriggerEvent;
		override public function execute():void
		{
			if ( event.type == LoadDashboardTriggerEvent.LOAD_FILE )
				var ee : LoadFile = new LoadFile( event.file, onLoaded, event.fxFault ) 
			if ( event.type == LoadDashboardTriggerEvent.LOAD_ARRAY )			
			{
				var series :   Array = []
				for each ( var i : IndicatorVO in this.event.array ) 
				{
					var new_series :  IndicatorVO = new IndicatorVO()
					new_series.series_id = i.series_id
					var obs : Array =  i.allDataPoints  
					if ( obs.length != 0 ) 
				...
				this.model.seriesAsArray = series; //event.array; 
			}
		...
		private function onLoaded(e: Event):void
		{
				...
				var list : XMLList = res.descendants( 'serie' ) ;
				var series :   Array = []
				...
				trace( 'parsed ' +  series.length + ' series ' ) ; 
				this.model.seriesAsArray = series; 
				
				
				this.openPopupFor( series[0] )
				return;
			}
			...
			{
				trace('data observations recieved ['+indicator.series_id+']' ) ; 
				var obs : Array = []; 
				var xml : XML = e as XML
				var observations : XMLList = xml.descendants( 'observation' ) ;
				
				obs = proc_parseObservations( observations )
				
				
				this.proc_Observations( indicator, obs )
			...
			private function proc_GetLastYearsData(i : IndicatorVO, reversed_orderered_observations :  Array ) : void
			{
				...
				//if less than three, get last 3
				if ( lastYears.length <= 1 ) 
				{
					for ( var y : int = 0; y < 3; y++ )
					{
						lastYears.push( 	reversed_orderered_observations[y] )
					}
				}
			...
			private function proc_Observations( i : IndicatorVO,  unordered_observations :  Array) : void
			{
				...
				ordered.sortOn( ['value'], [Array.NUMERIC] ) 
