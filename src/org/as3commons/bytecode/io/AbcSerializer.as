	...
	public class AbcSerializer {
		...
		public function serializeMethodBodies(abcFile:AbcFile):void {
			...
			for each (var body:MethodBody in methodBodies) {
				...
				trace( 'add body', count, body.methodSignature.methodName ); count++
		...
		public function serializeNamespaces(pool:IConstantPool, outputStream:ByteArray):void {
			...
			/*var namespaces2 : Array = [] //unique only ....
			var dict : Dictionary = new Dictionary(true); 
			for each (  namespaceInstance in namespaces) {
				if (dict[namespaceInstance.name]==null) //if not there 
				{
					dict[namespaceInstance.name] = 	 namespaceInstance.kind		
					namespaces2.push( namespaceInstance ) ; 	
					continue; 
				}
				if ( namespaceInstance.name != '' &&
					dict[namespaceInstance.name] == 	 namespaceInstance.kind	)
				{
					trace('skipping duplicate' ); 
				}
			...
			/*if ( pool.locked )
			{
				trace('pool locked') //03/08/12 ... wtf is this? 
				//pool.locked = false;
				return; 
			}*/
			...
			for each (var namespaceInstance:LNamespace in namespaces) {
				...
				if ( pool.locked )
				{
					...
					//pool.locked = false;
					//return; 
					if ( pool.stringPool.indexOf( namespaceInstance.name ) == -1 ) 
					{
						//the problem is it is not in the pool so i have to add it and increment the pool count ... which 
						//i cannot do ..
						trace('problem');
					}
				}
