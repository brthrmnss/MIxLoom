...
package org.mixingloom.patcher
{
	...
	public class MigrateFunctionsPatcher3_Test extends AbstractPatcher {
		...
		public function MigrateFunctionsPatcher3_Test(className:String, propertyOrMethodName:String)
		{
		...
		override public function apply( invocationType:InvocationType, swfContext:SwfContext ):void {
			...
			*/
			...
			var searchName : String = this.className.replace( reg , '/' ) 
			reg    = new RegExp("\\.", 'gi' ) ; 
			searchName = searchName.replace( reg , '/' ) 	
			for each (var swfTag:SwfTag in swfContext.swfTags)
			{
				...
				if (swfTag.type == DoABCTag.TAG_ID)
				{
					...
					//var abcFile:AbcFile = abcDeserializer.deserialize(4)
					//	trace('abclength', abcFile.methodBodies.length ) 
					//abcDeserializer.deserializeConstantPool(cp);
					//abcDeserializer.deserializeClassInfos( abcFile, cp, 10000000000000 ); 
